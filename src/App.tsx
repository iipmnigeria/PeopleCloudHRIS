...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      try {
        if (!user) { setCurrentUser(null); setViewMode((prev) => (prev === 'auth' ? 'auth' : 'landing')); return; }
        ...
        if (snap.exists()) {
          ...
          await setDoc(profileRef, profile); session = profile;
        }
        setCurrentUser(session); setViewMode('app'); if (session.companyId) setSelectedTenantId(session.companyId);
      } catch (error) { console.error('Auth sync state error:', error); }
