  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      try {
        ...
        console.error('Auth sync state error:', error);
      } finally {
        setLoading(false);
      }
    });
  ...
  useEffect(() => {
    ...
    async function loadCompanyName() {
      try {
        ...
        setCurrentCompanyName(snap.exists() ? snap.data().name : 'Demo Enterprise');
      } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row" id="app-workspace">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 max-h-screen overflow-y-auto">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {activeTab === 'analytics' && <HrAnalytics currentUser={currentUser} selectedTenantId={selectedTenantId} />}
          {activeTab === 'employees' && <EmployeeDb currentUser={currentUser} selectedTenantId={selectedTenantId} />}
          {activeTab === 'id-cards' && <IdCardStudio currentUser={currentUser} selectedTenantId={selectedTenantId} />}
