...
export default function EmployeeDataLinkManager({ currentUser, selectedTenantId }: Props) {
  ...
  async function loadData() {
    ...
    const linkSnap = await getDocs(collection(db, 'employee_data_links'));
    const linkRows: LinkRow[] = [];
    linkSnap.forEach((item) => { const data = item.data() as LinkRow; if (data.companyId === companyId) linkRows.push({ ...data, id: item.id }); });
    setLinks(linkRows);
    const subSnap = await getDocs(collection(db, 'employee_data_submissions'));
    const subRows: Submission[] = [];
    subSnap.forEach((item) => { const data = item.data() as Submission; if (data.companyId === companyId) subRows.push({ ...data, id: item.id }); });
  ...
  async function approveSubmission(item: Submission) {
    ...
    const data = { ...f, companyId, email: String(f.email || item.employeeEmail || '').toLowerCase(), bvn: f.bvn || '', bankVerificationNumber: f.bvn || f.bankVerificationNumber || '', nin: f.nin || '', nationalIdentificationNumber: f.nin || f.nationalIdentificationNumber || '', nextOfKinRelationship: f.relationship || f.nextOfKinRelationship || '', updatedAt: new Date().toISOString(), updatedBy: currentUser.uid };
  ...
  return <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4"><div className="flex items-center justify-between"><div><h3 className="text-sm font-black text-slate-900 flex items-center gap-2"><LinkIcon className="w-4 h-4 text-indigo-600" />Employee Data Collection Links</h3><p className="text-xs text-slate-500 mt-1">Generate secure links for new employee records or existing staff data updates.</p></div><button onClick={loadData} className="px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold flex items-center gap-2"><RefreshCw className="w-4 h-4" />Refresh</button></div>{message && <div className="rounded-xl bg-indigo-50 border border-indigo-100 p-3 text-xs font-bold text-indigo-800">{message}</div>}<div className="grid grid-cols-1 md:grid-cols-5 gap-3"><select value={purpose} onChange={(e) => setPurpose(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-xl text-xs bg-white"><option>New Employee Data Collection</option><option>Existing Staff Data Update</option></select><input value={employeeId} onChange={(e) => setEmployeeId(e.target.value)} placeholder="Employee ID, optional" className="px-3 py-2 border border-slate-200 rounded-xl text-xs" /><input value={employeeEmail} onChange={(e) => setEmployeeEmail(e.target.value)} placeholder="Employee email, optional" className="px-3 py-2 border border-slate-200 rounded-xl text-xs" /><input type="number" value={days} onChange={(e) => setDays(Number(e.target.value))} className="px-3 py-2 border border-slate-200 rounded-xl text-xs" /><button onClick={createLink} className="px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-black">Generate Link</button></div><div className="grid grid-cols-1 xl:grid-cols-2 gap-4"><div className="border border-slate-100 rounded-xl overflow-hidden"><div className="p-3 bg-slate-50 text-[10px] uppercase font-black text-slate-500">Generated Links</div>{links.slice(0, 5).map((link) => <div key={link.id} className="p-3 border-t border-slate-100 text-xs"><p className="font-bold text-slate-900">{link.title}</p><p className="text-[10px] text-slate-500">Expires: {link.expiresAt ? new Date(link.expiresAt).toLocaleDateString() : 'N/A'}</p><button onClick={() => navigator.clipboard.writeText(urlFor(link.id))} className="mt-2 px-2 py-1 rounded-lg bg-indigo-50 text-indigo-700 text-[10px] font-bold inline-flex gap-1 items-center"><Copy className="w-3 h-3" />Copy Link</button></div>)}</div><div className="border border-slate-100 rounded-xl overflow-hidden"><div className="p-3 bg-slate-50 text-[10px] uppercase font-black text-slate-500">Pending Submissions ({pending.length})</div>{pending.length === 0 && <p className="p-4 text-xs text-slate-400">No pending employee submissions.</p>}{pending.map((item) => <div key={item.id} className="p-3 border-t border-slate-100 text-xs"><p className="font-bold text-slate-900">{item.submittedData?.firstName} {item.submittedData?.lastName}</p><p className="text-[10px] text-slate-500">{item.employeeEmail || item.submittedData?.email} • {item.purpose}</p><button onClick={() => approveSubmission(item)} className="mt-2 px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-[10px] font-bold inline-flex gap-1 items-center"><CheckCircle2 className="w-3 h-3" />Approve & Update Database</button></div>)}</div></div></div>;
}
