...
import { Banknote, BarChart3, Calculator, CalendarCheck2, FileText, ShieldCheck } from 'lucide-react';
...
import BankPaymentScheduleExport from './BankPaymentScheduleExport';
import MultiCountryPayrollDashboard from './MultiCountryPayrollDashboard';

export default function GlobalPayrollWorkspace(props: any) {
  const isEmployee = props?.currentUser?.role === 'Employee';
  const [tab, setTab] = useState<'dashboard' | 'engine' | 'approval' | 'payslips' | 'attendance' | 'bank'>(isEmployee ? 'payslips' : 'dashboard');
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-slate-200 p-2 flex flex-wrap gap-2">
        <button onClick={() => setTab('dashboard')} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold ${tab === 'dashboard' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}><BarChart3 className="w-4 h-4" />Multi-Country Dashboard</button>
        ...
        <button onClick={() => setTab('bank')} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold ${tab === 'bank' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}><Banknote className="w-4 h-4" />Bank Payment Export</button>
      ...
      {tab === 'dashboard' && <MultiCountryPayrollDashboard {...props} />}
      ...
      {tab === 'bank' && <BankPaymentScheduleExport {...props} />}
